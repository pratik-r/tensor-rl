...
if __name__ == "__main__":
    ...
    for gamma in gamma_seq:
        ...
        for seed in tqdm(range(niter)):
            out[gamma] += [run(seed,alpha_dirs,Xi,nactions,d1,d2,d3,thetahat_qlearn,R1,n_episodes,episode_length,n_samples,gamma,eps_policy,noise)]
            
    df = pd.concat({k: pd.DataFrame(v) for k,v in out.items()}).stack().reset_index(level=[0,2]).rename(columns = {'level_0':'gamma','level_2':'method',0:'value_estimation_error'}).reset_index(drop = True)    
    
    plt.rcParams['figure.figsize'] = (16,9)
    plt.rcParams['figure.facecolor'] = 'white'
    plot = sns.catplot(kind='box', x='method', y='value_estimation_error', col='gamma', showfliers=False, sharey = False, data=df)
    plot.savefig('compare_methods_gamma.png',bbox_inches='tight')