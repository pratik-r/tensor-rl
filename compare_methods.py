...
def run(seed,alpha_dirs,Xi,nactions,d1,d2,d3,thetahat_qlearn,R1,n_episodes,episode_length,n_samples,gamma,eps_policy,noise):
    ...
    out = dict()
    out['tensor_IVreg'] = np.linalg.norm(values_mc - values_tIVreg) / np.sqrt(n_samples)
...
if __name__ == "__main__":
    ...
    for R1 in R1_seq:
        ...
        for seed in tqdm(range(niter)):
            out[R1+nactions] += [run(seed,alpha_dirs,Xi,nactions,d1,d2,d3,thetahat_qlearn,R1,n_episodes,episode_length,n_samples,gamma,eps_policy,noise)]
            
    df = pd.concat({k: pd.DataFrame(v) for k,v in out.items()}).stack().reset_index(level=[0,2]).rename(columns = {'level_0':'rank','level_2':'method',0:'value_estimation_error'}).reset_index(drop = True)    
    
    plt.rcParams['figure.figsize'] = (16,9)
    plt.rcParams['figure.facecolor'] = 'white'
    plot = sns.catplot(kind='box', x='method', y='value_estimation_error', col='rank', data=df)
    plot.savefig('compare_methods.png',bbox_inches='tight')